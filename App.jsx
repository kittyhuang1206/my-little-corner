...
const SYNC_DUMP=()=>{const o={};try{for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(k&&k.indexOf("mlc_")===0)o[k]=localStorage.getItem(k);}}catch{}delete o.mlc_photo;delete o.mlc_photo_pushed_v1;try{if(o.mlc_h){const h=JSON.parse(o.mlc_h);let ch=false;Object.keys(h).forEach(k=>{if(h[k]&&h[k].photo){h[k]={...h[k]};delete h[k].photo;ch=true;}});if(ch)o.mlc_h=JSON.stringify(h);}}catch(e){}return o;};
const SYNC_WRITE=(o)=>{try{Object.keys(o||{}).forEach(k=>{if(k.indexOf("mlc_")!==0||typeof o[k]!=="string")return;if(k==="mlc_photo"||k==="mlc_photo_pushed_v1")return;if(k==="mlc_h"){try{const inc=JSON.parse(o[k]||"{}");const loc=JSON.parse(localStorage.getItem("mlc_h")||"{}");const merged={...loc};Object.keys(inc).forEach(dk=>{const ie=inc[dk]||{};const le=loc[dk]||{};merged[dk]={...le,...ie,photo:(ie.photo||le.photo||"")};});localStorage.setItem("mlc_h",JSON.stringify(merged));}catch(e){localStorage.setItem(k,o[k]);}return;}localStorage.setItem(k,o[k]);});}catch{}};
const LOCAL_TS=()=>{try{return Number(localStorage.getItem("mlc_ts")||0);}catch{return 0;}};
const SYNC_RELOAD=()=>{try{const last=Number(sessionStorage.getItem("mlc_reloaded_at")||0);if(Date.now()-last<10000)return;sessionStorage.setItem("mlc_reloaded_at",String(Date.now()));}catch(e){}try{location.reload();}catch(e){}};
const pad2=n=>String(n).padStart(2,"0");
const localKey=d=>d.getFullYear()+"-"+pad2(d.getMonth()+1)+"-"+pad2(d.getDate());
const todayKey=()=>localKey(new Date());
const dAgo=n=>localKey(new Date(Date.now()-n*864e5));
const compressImage=(srcUrl,maxDim=1100,quality=0.72)=>new Promise(resolve=>{try{const img=new Image();img.onload=()=>{try{let w=img.naturalWidth||img.width,h=img.naturalHeight||img.height;const sc=Math.min(1,maxDim/Math.max(w,h));w=Math.max(1,Math.round(w*sc));h=Math.max(1,Math.round(h*sc));const c=document.createElement("canvas");c.width=w;c.height=h;const ctx=c.getContext("2d");ctx.drawImage(img,0,0,w,h);const out=c.toDataURL("image/jpeg",quality);resolve(out&&out.length>50?out:srcUrl);}catch(e){resolve(srcUrl);}};img.onerror=()=>resolve(srcUrl);img.src=srcUrl;}catch(e){resolve(srcUrl);}});
const getHistory=()=>S.get("mlc_h",{});
const saveDay=d=>{const h=getHistory();const prev=h[d.date]||{};const nt=d.tasks||[];const ids=new Set(nt.map(x=>x.id));const tasks=[...nt,...(prev.tasks||[]).filter(x=>!ids.has(x.id))];h[d.date]={...prev,...d,tasks,win:d.win||prev.win||"",focus:d.focus||prev.focus||"",vibe:d.vibe||prev.vibe||"",photo:d.photo||prev.photo||""};S.set("mlc_h",h);};
...
const TRACKS={
  ...
  bookstore:[
    ...
    {t:"String Quartet",u:"/audio/string-quartet.mp3"},
  ],
  evening:[
...
export default function App(){
  ...
  useEffect(()=>{clearTimeout(asT.current);asT.current=setTimeout(()=>saveDay({date:(S.get("mlc_day","")||todayKey()),tasks,win,focus,vibe,photo,quote,companion:compId,season}),800);},[tasks,win,focus,vibe,photo]);
  const dayRef=useRef(null);
  dayRef.current={tasks,win,focus,vibe,photo,quote,compId,season};
  useEffect(()=>{if(!toast)return;const id=setTimeout(()=>setToast(""),2800);return ()=>clearTimeout(id);},[toast]);
  // auto day rollover: archive yesterday + start fresh, at midnight / on reopen
  useEffect(()=>{
    const check=()=>{
      const tk=todayKey();const cd=S.get("mlc_day","");
      if(!cd){S.set("mlc_day",tk);return;}
      if(cd===tk)return;
      const d=dayRef.current||{};
      const hasContent=(d.tasks||[]).some(x=>x.done)||d.win||d.photo||d.focus||d.vibe;
      if(hasContent)saveDay({date:cd,tasks:d.tasks||[],win:d.win||"",focus:d.focus||"",vibe:d.vibe||"",photo:d.photo||"",quote:d.quote||"",companion:d.compId,season:d.season});
      setTasks(p=>p.filter(x=>!x.done));setWin("");setWinDraft("");setFocus("");setFocusDraft("");setVibe("");setPhoto("");
      S.set("mlc_day",tk);
      const zh=S.get("mlc_lang","zh")==="zh";
      setToast(zh?"新的一天開始了，昨天已收進日記 ✿":"A new day! Yesterday is safe in your journal ✿");
    };
    check();
  ...
  // one-time: shrink old oversized photos so saving & syncing work again
  useEffect(()=>{let live=true;(async()=>{try{const h=getHistory();let ch=false;for(const k of Object.keys(h)){const ph=h[k]&&h[k].photo;if(ph&&ph.length>300000){const c=await compressImage(ph);if(c&&c.length<ph.length){h[k]={...h[k],photo:c};ch=true;}}}if(ch&&live)S.set("mlc_h",h);const cur=S.get("mlc_photo","");if(cur&&cur.length>300000){const c2=await compressImage(cur);if(live&&c2&&c2.length<cur.length)setPhoto(c2);}}catch(e){}})();return ()=>{live=false;};},[]);
  ...
  const confirmNextDay=()=>{const cd=S.get("mlc_day","")||todayKey();saveDay({date:cd,tasks,win,focus,vibe,photo,quote,companion:compId,season});triggerComp("nextDay","bounce");setTasks(p=>p.filter(x=>!x.done));setWin("");setWinDraft("");setFocus("");setFocusDraft("");setVibe("");setPhoto("");setShowNextDay(false);S.set("mlc_day",todayKey());loadTr(pickT(musicCat));};
  const pdot=p=>({low:P.sage,medium:P.brown,high:P.peach}[p]||P.muted);
  const handlePhoto=e=>{const file=e.target.files?.[0];if(!file)return;const r=new FileReader();r.onload=ev=>{compressImage(String(ev.target.result)).then(img=>{setPhoto(img);setToast(l==="zh"?"照片已收進今天的日記 ♡":"Photo saved to today\'s journal ♡");});};r.readAsDataURL(file);try{e.target.value="";}catch(err){}};
  const clearPhoto=()=>{const cd=S.get("mlc_day","")||todayKey();setPhoto("");try{const h=getHistory();if(h[cd]&&h[cd].photo){h[cd]={...h[cd],photo:""};S.set("mlc_h",h);}}catch(e){}if(typeof window!=="undefined"&&window.__mlcPhotoDel)window.__mlcPhotoDel(cd);};
  ...
  useEffect(()=>{ if(!_fbDb||!syncUser){if(typeof window!=="undefined"){window.__mlcPush=null;window.__mlcPhotoDel=null;}return;}
    const ref=doc(_fbDb,"states",syncUser.uid);
    const photosCol=collection(_fbDb,"states",syncUser.uid,"photos");
    const isEmpty=(o)=>{try{const tk=JSON.parse((o&&o["mlc_tasks"])||"[]");const h=JSON.parse((o&&o["mlc_h"])||"{}");return (!tk||tk.length===0)&&(!h||Object.keys(h).length===0);}catch(e){return true;}};
    const getPushed=()=>{try{return JSON.parse(localStorage.getItem("mlc_photo_pushed_v1")||"{}");}catch(e){return {};}};
    const setPushed=(m)=>{try{localStorage.setItem("mlc_photo_pushed_v1",JSON.stringify(m));}catch(e){}};
    // photos sync one-per-day in their own documents (keeps each under Firestore's 1MB limit)
    const pushPhotos=async()=>{try{const h=getHistory();const pushed=getPushed();let dirty=false;for(const date of Object.keys(h)){const ph=h[date]&&h[date].photo;if(ph&&ph.length>0&&ph.length<900000&&pushed[date]!==ph.length){await setDoc(doc(photosCol,date),{img:ph,ts:Date.now()});pushed[date]=ph.length;dirty=true;}}if(dirty)setPushed(pushed);}catch(e){}};
    const pullPhotos=async()=>{try{const snap=await getDocs(photosCol);const h=getHistory();const pushed=getPushed();let changed=false;snap.forEach(dc=>{const date=dc.id;const img=(dc.data()||{}).img;if(!img)return;if(!h[date]){h[date]={date,tasks:[],win:"",focus:"",vibe:"",photo:img};changed=true;pushed[date]=img.length;}else if(!h[date].photo){h[date]={...h[date],photo:img};changed=true;pushed[date]=img.length;}});if(changed){try{localStorage.setItem("mlc_h",JSON.stringify(h));}catch(e){}setPushed(pushed);const tk=S.get("mlc_day","")||todayKey();const te=h[tk];if(te&&te.photo)setPhoto(prev=>prev||te.photo);}}catch(e){}};
    window.__mlcPhotoDel=(date)=>{try{deleteDoc(doc(photosCol,date));const m=getPushed();delete m[date];setPushed(m);}catch(e){}};
    const push=()=>{setDoc(ref,{data:SYNC_DUMP(),ts:LOCAL_TS(),uid:syncUser.uid},{merge:true}).then(()=>{setSyncMsg("ok");pushPhotos();}).catch((e)=>{setSyncMsg("err");try{console.warn("sync push failed",e&&e.code,e);}catch(_){}});};
    window.__mlcPush=()=>{clearTimeout(pushT.current);pushT.current=setTimeout(push,1200);};
    pullPhotos();
    let firstSnap=true;
    const unsub=onSnapshot(ref,snap=>{ const d=snap.exists()?snap.data():null; const cloud=d&&d.data?d.data:null; const local=SYNC_DUMP(); const same=cloud&&JSON.stringify(cloud)===JSON.stringify(local); if(same){firstSnap=false;setSyncMsg("ok");pushPhotos();return;} if(!cloud||isEmpty(cloud)){firstSnap=false;if(!isEmpty(local))push();return;} if(isEmpty(local)){firstSnap=false;SYNC_WRITE(cloud);pullPhotos().then(()=>SYNC_RELOAD());return;} if(Number(d.ts||0)>LOCAL_TS()){firstSnap=false;SYNC_WRITE(cloud);pullPhotos().then(()=>SYNC_RELOAD());}else{firstSnap=false;push();} },()=>setSyncMsg("err"));
    return ()=>{if(typeof window!=="undefined"){window.__mlcPush=null;window.__mlcPhotoDel=null;}try{unsub&&unsub();}catch{}};
  },[syncUser]);
  const authErr=(e)=>{const code=(e&&e.code)||"";const zh=l==="zh";if(code==="auth/email-already-in-use")return zh?"此 Email 已註冊，請改用「登入」。":"Email already registered — use Log in.";if(code==="auth/invalid-email")return zh?"Email 格式不正確。":"Invalid email.";if(code==="auth/weak-password")return zh?"密碼至少要 6 個字。":"Password needs 6+ characters.";if(code==="auth/invalid-credential"||code==="auth/wrong-password"||code==="auth/user-not-found")return zh?"Email 或密碼錯誤。":"Wrong email or password.";if(code==="auth/network-request-failed")return zh?"網路連線失敗。":"Network error.";if(code==="auth/too-many-requests")return zh?"嘗試太多次，已被暫時鎖住。請等幾分鐘，或用下方「忘記密碼?」重設。":"Too many attempts — locked briefly. Wait a few minutes or use Forgot password.";if(code==="auth/operation-not-allowed")return zh?"請先在 Firebase 開啟 Email/密碼登入。":"Enable Email/Password sign-in in Firebase first.";return zh?"操作失敗，請再試一次。":"Something went wrong, try again.";};
  ...
  return(
    <div style={{background:P.bg,minHeight:"100vh"}}>
            ...
            {syncUser?(
              ...
              <div style={{marginBottom:18}}>
                ...
                <div style={{display:"flex",gap:7}}>
                  <button className="btn" style={{flex:1,fontSize:13}} onClick={doEmailSignup}>{l==="zh"?"建立帳號":"Sign up"}</button>
                  <button className="btn-o" style={{flex:1,fontSize:13}} onClick={doEmailLogin}>{l==="zh"?"登入":"Log in"}</button>
                </div>
            ...
            <div style={{background:P.card,borderTop:`1px solid ${P.border}`,padding:"11px 17px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              ...
              <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:5}}>
                <div style={{display:"flex",gap:5,alignItems:"center"}}>
                  {syncUser&&<div title={syncMsg==="err"?(l==="zh"?"同步發生問題":"Sync problem"):(l==="zh"?"同步中":"Synced")} style={{background:P.light,border:`1px solid ${syncMsg==="err"?"#E0B8B8":P.border}`,borderRadius:50,padding:"2px 8px",fontSize:10,color:syncMsg==="err"?"#C07060":P.sub,fontFamily:font.sans}}>{syncMsg==="err"?"☁ !":"☁ ✓"}</div>}
              ...
              <div className="card" style={{padding:photo?"11px":"18px 20px"}}>
                {photo?(
                  <div style={{position:"relative"}}>
                    ...
                    <button onClick={clearPhoto} style={{position:"absolute",top:8,right:8,background:"rgba(47,52,58,.58)",border:"none",borderRadius:6,color:"white",cursor:"pointer",padding:"3px 8px",fontSize:12}}>✕</button>
                  </div>
                ):(
      ...
      {toast&&<div style={{position:"fixed",bottom:88,left:"50%",transform:"translateX(-50%)",background:"rgba(47,52,58,.84)",color:"#FFFDF8",borderRadius:50,padding:"9px 18px",fontSize:12.5,fontFamily:font.sans,zIndex:700,animation:"fadeUp .25s ease",maxWidth:"86%",textAlign:"center",boxShadow:"0 4px 14px rgba(47,52,58,.25)"}}>{toast}</div>}
