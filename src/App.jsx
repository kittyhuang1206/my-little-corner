...
export default function App(){
  ...
  useEffect(()=>{ if(!_fbDb||!syncUser){if(typeof window!=="undefined")window.__mlcPush=null;return;} const ref=doc(_fbDb,"states",syncUser.uid); const isEmpty=(o)=>{try{const tk=JSON.parse((o&&o["mlc_tasks"])||"[]");const h=JSON.parse((o&&o["mlc_h"])||"{}");return (!tk||tk.length===0)&&(!h||Object.keys(h).length===0);}catch(e){return true;}}; const push=()=>{setDoc(ref,{data:SYNC_DUMP(),ts:LOCAL_TS(),uid:syncUser.uid},{merge:true}).then(()=>setSyncMsg("ok")).catch(()=>setSyncMsg("err"));}; window.__mlcPush=()=>{clearTimeout(pushT.current);pushT.current=setTimeout(push,1200);}; let firstSnap=true; const unsub=onSnapshot(ref,snap=>{ const d=snap.exists()?snap.data():null; const cloud=d&&d.data?d.data:null; const local=SYNC_DUMP(); const same=cloud&&JSON.stringify(cloud)===JSON.stringify(local); if(same){firstSnap=false;setSyncMsg("ok");return;} if(!cloud||isEmpty(cloud)){firstSnap=false;if(!isEmpty(local))push();return;} if(isEmpty(local)){firstSnap=false;SYNC_WRITE(cloud);SYNC_RELOAD();return;} if(firstSnap){firstSnap=false;push();return;} if(Number(d.ts||0)>LOCAL_TS()){SYNC_WRITE(cloud);SYNC_RELOAD();}else{push();} },()=>setSyncMsg("err")); return ()=>{if(typeof window!=="undefined")window.__mlcPush=null;try{unsub&&unsub();}catch{}}; },[syncUser]);
